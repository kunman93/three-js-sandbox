...
let renderer: any, camera: any, scene: any, cube: any, controls: any;

/**
 *  ---- Cube animation in Three.js ---- 
 */
main();
...
function main() {
  ...
  // # Creating a cube
  // ## Creating a box geometry - the shape of the object -, which contains the data of the box
  const boxWidth = 1;
  const boxHeight = 1;
  const boxDepth = 1;
  const geometry = new THREE.BoxGeometry(boxWidth, boxHeight, boxDepth);

  // ## Creating a mesh phong material (shiny or flat, what color, what textures to apply, etc.) which is affected by the light
  const material = new THREE.MeshPhongMaterial({ color: 0x44aa88 });

  // ## Creating a mesh
  /**
   * a mesh represents the combination of three things: 
   * - Geometry
   * - Material 
   * - position, orientation, and scale of that object in the scene relative to its parent
  */
  cube = new THREE.Mesh(geometry, material);

  // ## Add newly created cube to the scene
  scene.add(cube);
  ...
  controls.minDistance = 1.5; // how far you can dolly in
  controls.maxDistance = 3; // how far you can dolly out
